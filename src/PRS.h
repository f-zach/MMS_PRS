...
private:
    ...
    byte _default_i2c_address[8] = {0,0,0,0,0,0,0,0};

public:
    PRSmodule(int i2cAddress);
    void config(byte sensorPositions, float PressureRange[8], byte diff, int UnitSensor[8], int UnitReq[8], byte sensor_i2c_address[8], bool checkFault = false, int errLED = 5, int busyLED = 6);
